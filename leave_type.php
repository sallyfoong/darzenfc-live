...
$sub_db = 'leave_type'; 
$back_url = SITEURL."/leave_type_list.php";
$error_message = '';
$mode = input('mode');
$txtgetID = input('id');
...
if(isset($_POST['save'])){
    ...
    $txtdatetime = strtotime($txtdate);
    $yearString = date('Y', $txtdatetime);
    if($name && $txtdate){
        if(mysqli_num_rows(mysqli_query($connect,"SELECT id FROM ".$sub_db." WHERE leave_name = '".$name."' AND leave_date='".$txtdate."' AND status = 'A'"))==0){
            $dataInfo = "INSERT INTO ".$sub_db." (leave_name, leave_date, year, create_date, create_time, create_by, status) VALUES ('".$name."', '".$txtdate."', '".$yearString."', '".$cdate."', '".$ctime."', '".$cby."', 'A')";
...
if(isset($_POST['edit'])){
    ...
    $org_txtdate = $_POST['org_txtdate'];
    
    $txtdatetime = strtotime($txtdate);
    $yearString = date('Y', $txtdatetime);
    
    if(($name != $org_txtname) || ($txtdate != $org_txtdate)){
      if(mysqli_num_rows(mysqli_query($connect,"SELECT id FROM ".$sub_db." WHERE leave_name = '".$name."' AND leave_date='".$txtdate."' AND status = 'A'"))==0){
          $sqlupd = "UPDATE ".$sub_db." SET leave_name = '".$name."', leave_date = '".$txtdate."', year = '".$yearString."', update_date = '".$cdate."', update_time = '".$ctime."', update_by = '".$cby."' WHERE id = '".$txtid."' AND status = 'A'";
          ...
          if ($query2){
              echo '<script language="javascript">';
                ...
                $old_value2 = $org_txtdate;
                $new_value2 = $txtdate;
                if($old_value2 == $new_value2){ $changes2 = ''; }else{ $changes2 = "Leave Date: ".$old_value2." to ".$new_value2; }
...
if($mode == 'edit'){
    $datainfo = "SELECT `id`, `leave_name`, `leave_date`, `create_date`, `create_time`, `create_by`, `update_date`, `update_time`, `update_by`, `status` FROM ".$sub_db." WHERE id ='".$txtgetID."' AND STATUS = 'A' LIMIT 1 ";
    $rowRetrieveInfo = mysqli_query($connect, $datainfo);
    $data = mysqli_fetch_assoc($rowRetrieveInfo);
    $txtid = $data['id'];
    $name = $data['leave_name'];
    $txtdate = $data['leave_date'];
}
?>
<!DOCTYPE html>
...
jQuery(document).ready(function() {
    jQuery("form").submit(function() {
        ...
        const txtdate = jQuery('#txtdate');
        if (txtname.val() == "") {
            alert("Please fill the Name");
            txtname.focus();
            return false;
        }
        if (txtusernametxtdate.val() == "") {
                            ...
                            <div class="card-body">
                                ...
                                <form id="epfform" method="POST">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="txtname" id="txtname" class="form-control demo"
                                            data-control="hue" required autofocus value="<?php echo $name; ?>" />
                                        <input type="text" name="org_txtname" value="<?php echo $name; ?>"
                                            class="dnone">
                                    </div>
                                    <div class="form-group">
                                        <label for="position-bottom-left">Date</label>
                                        <input type="date" name="txtdate" id="txtdate" class="form-control demo"
                                            data-position="bottom left" value="<?php echo $txtdate; ?>" required />
                                        <input type="text" name="org_txtdate" value="<?php echo $txtdate; ?>"
                                            class="dnone">
                                    </div>
                                    <div>
                                        <?php if($mode != 'edit'){ ?>
