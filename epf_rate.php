...
$page_title = 'EPF Rate';
$sub_db = 'epf_rate'; 
$back_url = SITEURL."/epf_rate_list.php";
$error_message = '';
$mode = input('mode');
$txtgetID = input('id');
...
if(isset($_POST['save'])){
    ...
    if($name && $employer_rate && $employee_rate){
        if(mysqli_num_rows(mysqli_query($connect,"SELECT id FROM ".$sub_db." WHERE name = '".$name."' AND employer_epf_rate='".$employer_rate."' AND employee_epf_rate='".$employee_rate."' AND status = 'A'"))==0){
            $dataInfo = "INSERT INTO ".$sub_db." (name, employer_epf_rate, employee_epf_rate, create_date, create_time, create_by, status) VALUES ('".$name."', '".$employer_rate."', '".$employee_rate."','".$cdate."', '".$ctime."', '".$cby."', 'A')";
            ...
            $changes2   = "Employer EPF Rate: ".$new_value2;
            
            $new_value3 = $txtemployee;
            $changes3   = "Employee EPF Rate: ".$new_value3;
...
if(isset($_POST['edit'])){
    ...
    if($name && $employer_rate && $employee_rate){
      ...
      if(mysqli_num_rows(mysqli_query($connect,"SELECT id FROM ".$sub_db." WHERE name = '".$name."' AND employer_epf_rate='".$employer_rate."' AND employee_epf_rate='".$employee_rate."' AND status = 'A'"))==0){
          $sqlupd = "UPDATE ".$sub_db." SET name = '".$name."', employer_epf_rate = '".$employer_rate."', employee_epf_rate = '".$employee_rate."', update_date = '".$cdate."', update_time = '".$ctime."', update_by = '".$cby."' WHERE id = '".$txtid."' AND status = 'A'";
          ...
          if ($query2){
              echo '<script language="javascript">';
                ...
                if($old_value2 == $new_value2){ $changes2 = ''; }else{ $changes2 = "Employee EPF Rate: ".$old_value2." to ".$new_value2; }
                
                $old_value3 = $org_txtemployer;
                $new_value3 = $employer_rate;
                if($old_value3 == $new_value3){ $changes3 = ''; }else{ $changes3 = "Employer EPF Rate: ".$old_value3." to ".$new_value3; }
...
if($mode == 'edit'){
    $datainfo = "SELECT `id`, `name`, `employer_epf_rate`, `employee_epf_rate`, `create_date`, `create_time`, `create_by`, `update_date`, `update_time`, `update_by`, `status` FROM ".$sub_db." WHERE id ='".$txtgetID."' AND STATUS = 'A' LIMIT 1 ";
    $rowRetrieveInfo = mysqli_query($connect, $datainfo);
    $data = mysqli_fetch_assoc($rowRetrieveInfo);
    $txtid = $data['id'];
    $txtname = $data['name'];
    $txtemployer = $data['employer_epf_rate'];
    $txtemployee = $data['employee_epf_rate'];
}
?>
<!DOCTYPE html>
...
jQuery(document).ready(function() {
    jQuery("form").submit(function() {
        ...
        if (jQuery('#txtemployer').val() == "") {
            alert("Please fill the Employer EPF Rate");
            return false;
        } else if (isNaN(jQuery('#txtemployer').val())) {
            alert("Please fill the Employer EPF Rate with number only");
            return false;
        }
        if (jQuery('#txtemployee').val() == "") {
            alert("Please fill the Employee EPF Rate");
            return false;
        } else if (isNaN(jQuery('#txtemployee').val())) {
            alert("Please fill the Employee EPF Rate with number only");
            return false;
        }
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form id="epfform" method="POST">
                                    ...
                                    <div class="form-group">
                                        <label for="position-bottom-left">Employer EPF Rate (%)</label>
                                    ...
                                    <div class="form-group">
                                        <label for="position-top-right">Employee EPF Rate (%)</label>
