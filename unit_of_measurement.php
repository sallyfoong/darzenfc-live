...
$page_action = 'Add';
$page_title = 'Unit of Measurement';
$sub_db = 'unit_of_measurement'; 
$back_url = SITEURL."/unit_of_measurement_list.php";
$error_message = '';
$mode = input('mode');
$txtgetID = input('id');
...
if(isset($_POST['save'])){
    ...
    if($name){
        if(mysqli_num_rows(mysqli_query($connect,"SELECT id FROM ".$sub_db." WHERE name = '".$name."' AND remark='".$remark."' AND status = 'A'"))==0){
            $dataInfo = "INSERT INTO ".$sub_db." (name, remark, create_date, create_time, create_by, status) VALUES ('".$name."', '".$remark."', '".$cdate."', '".$ctime."', '".$cby."', 'A')";
...
if($mode == 'edit'){
    $datainfo = "SELECT `id`, `name`, `remark`, `create_date`, `create_time`, `create_by`, `update_date`, `update_time`, `update_by`, `status` FROM ".$sub_db." WHERE id ='".$txtgetID."' AND STATUS = 'A' LIMIT 1 ";
...
jQuery(document).ready(function() {
    jQuery("form").submit(function() {
        if (jQuery('#txtname').val() == "") {
            alert("Please fill the Unit of Measurement");
            return false;
        }
                            ...
                            <div class="card-body">
                                ...
                                <form id="epfform" method="POST">
                                    ...
                                    <div class="form-group">
                                        <label for="hue-demo">Unit of Measurement</label>
                                    ...
                                    <div class="form-group">
                                        <label for="position-bottom-left">Remark</label>
                                        <input type="text" name="remark" id="txtremark" class="form-control demo"
                                            data-position="bottom left" value="<?php echo $remark; ?>" />
                                        <input type="text" name="org_txtremark" value="<?php echo $remark; ?>"
                                            class="dnone">
                                    </div>
                                    <div>
                                        <?php if($mode != 'edit'){ ?>
